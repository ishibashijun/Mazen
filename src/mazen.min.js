...
a.width-h-1),q=MathUtils.randomOddInt(1,a.height-g-1),m=new Room(l,d,q,h,g),n=!1;0!==a.roomLength()&&a.rooms.forEach(function(a,b,c){return a.overlaps(m)?(n=!0,!1):!0});if(!n)if(a.roomLength()<a.parameters.maxRooms)m.id=a.getDungeonManager().getRoomId(),a.setRoom(m),this.createRoomMap(a,m),a.parameters.createExtraCombinedRooms||this.createRoomWallMap(a,m);else return}a.roomLength()<a.minRooms&&this.createRooms(a,b+1)}},createRoomMap:function(a,b){for(var c=b.x+b.width,e=b.y+b.height,f=b.y,k=e-1,h=
c-1,g=b.x,d=b.y;d<e;d++)for(var q=b.x;q<c;q++){var m=a.getCell(q,d);b.setCell(m);a.updateMap(Terrain.ROOM,b.id,q,d);d===f&&b.north.push(m);d===k&&b.south.push(m);q===h&&b.east.push(m);q===g&&b.west.push(m)}},createRoomWallMap:function(a,b){for(var c=b.x+b.width+1,e=b.y+b.height+1,f=b.x-1,k=c-1,h=b.y-1,g=e-1,d=b.y-1;d<e;d++)for(var q=b.x-1;q<c;q++)if(d===h||d===g||q===f||q===k){var m=new Wall(a.getDungeonManager().getWallId(),q,d);a.updateMap(Terrain.WALL,m.id,q,d);a.setWall(m)}},createExtraRoomsToCombine:function(a){var b=
...
h)),d--):f++}},createAllRoomWallMap:function(a){a.rooms.forEach(function(b,c,e){b=[b.north,b.south,b.east,b.west];for(var f in b)if(c=b[f],0!=c.length)for(var d in c){e=c[d];for(var h in Direction.BOX){var g=e.clone().add(Direction.BOX[h]);if(a.getMap(g.x,g.y)===Terrain.NOTHING){var p=new Wall(a.getDungeonManager().getWallId(),g.x,g.y);a.updateMap(Terrain.WALL,p.id,g.x,g.y);a.setWall(p)}}}return!0})},createCorridors:function(a){for(var b=a.width-1,c=a.height-1,e=1;e<c;e+=2)for(var f=1;f<b;f+=2)if(a.getMap(f,
e)===Terrain.NOTHING){var d=new Corridor(a.getDungeonManager().getCorridorId()),h=a.getCell(f,e);d.setCell(h);a.setCorridor(d);this.startMaze(a,d,h)}},startMaze:function(a,b,c){var e=[],f=l,d=a.parameters.branchOut?a.parameters.corridorBranchOutRatio:-1,h=[Direction.NORTH2,Direction.SOUTH2,Direction.EAST2,Direction.WEST2];for(e.push(c);0<e.length;){var g=e[e.length-1],p=[];c=l;ArrayUtils.shuffle(h);a.updateMap(Terrain.CORRIDOR,b.id,g.x,g.y);for(var q in h)this.canAdvanse(a,g,h[q])&&p.push(h[q]);0!=
p.length?d<Math.random()?(c=this.arrayContainsDirection(p,f)&&a.parameters.corridorCurveRatio<=Math.random()?f:p[MathUtils.randomInt(p.length-1)],p=(new Cell(.5*c.x,.5*c.y)).add(g),f=a.getCell(p.x,p.y),g.setCell(f),f.setCell(g),a.updateMap(Terrain.CORRIDOR,b.id,p.x,p.y),g=(new Cell(c.x,c.y)).add(g),g=a.getCell(g.x,g.y),f.setCell(g),g.setCell(f),e.push(g),b.setCell(f),b.setCell(g),f=c):(f=e.pop(),1>=f.cellLength()&&(f.deadEnd=!0,b.setDeadEnd(f)),f=l):(f=e.pop(),1>=f.cellLength()&&(f.deadEnd=!0,b.setDeadEnd(f)),
f=l)}},arrayContainsDirection:function(a,b){if(b==l)return!1;var c,e=a.length;for(c=0;c<e;c++)if(a[c].x===b.x&&a[c].y===b.y)return!0;return!1},canAdvanse:function(a,b,c){var e=b.x+c.x;b=b.y+c.y;return 0>e||a.width<=e||0>b||a.height<=b?!1:a.getMap(e,b)===Terrain.NOTHING?!0:!1},connectCorridors:function(a){var b=this;a.corridors.forEach(function(c,e,f){var d={},h=0;c.cells.forEach(function(e,f,q){e=b.getNeighbourCorridors(a,c,e);for(var m in e)f=e[m],d.hasOwnProperty(m)?d[m].cells=d[m].cells.concat(f.cells):
(d[m]=f,h++);return!0});0<h&&(c.corridorGroup==l&&(c.corridorGroup=new CorridorGroup(a.getDungeonManager().getCorridorGroupId()),c.corridorGroup.setCorridor(c),a.setCorridorGroup(c.corridorGroup)),b.updateCorridorGroup(a,c,d));return!0})},getNeighbourCorridors:function(a,b,c){var e={},f;for(f in Direction.CROSS2){var d=Direction.CROSS2[f].x,h=Direction.CROSS2[f].y,g=c.clone().add(Direction.CROSS2[f]);if(a.getMap(g.x,g.y)===Terrain.CORRIDOR){var p=l,q=l,m=a.getDungeonManager().getMap(g.x,g.y);m.id==
b.id||b.isCorridorConnected(a.getCorridor(m.id))||(p=a.getCorridor(m.id),q=p.getCell(g.x,g.y));p!=l&&(e.hasOwnProperty(p.id.toString())?e[p.id.toString()].cells.push(new CellData(new Cell(d,h),c,q)):e[p.id.toString()]={corridor:p,cells:[new CellData(new Cell(d,h),c,q)]})}}return e},updateCorridorGroup:function(a,b,c){for(var e in c){var f=c[e].corridor,d=c[e].cells;if(!b.corridorGroup.isCorridorConnected(f)){var h=MathUtils.randomInt(0,d.length-1),g=d[h].dir,p=d[h].dstCell,d=d[h].srcCell,g=d.clone().add(g.div(2)),
h=a.getCell(g.x,g.y);b.setCell(h);a.updateMap(Terrain.CORRIDOR,b.id,g.x,g.y);d.setCell(h);p.setCell(h);h.setCell(d);h.setCell(p);p.deadEnd&&1<p.cellLength()&&(f.deleteDeadEnd(p),f.corridorGroup!=l&&f.corridorGroup.deleteDeadEnd(p),p.deadEnd=!1);d.deadEnd&&1<d.cellLength()&&(b.deleteDeadEnd(d),b.corridorGroup.deleteDeadEnd(d),d.deadEnd=!1);f.corridorGroup!=l?(f=f.corridorGroup,a.deleteCorridorGroup(f.id),b.corridorGroup.combine(f),f.dispose()):b.corridorGroup.setCorridor(f)}}},createEntrance:function(a){var b=
a.width-1,c=a.height-1,e=this;a.corridors.forEach(function(d,k,h){d.cells.forEach(function(g,h,k){if(d.roomLength()===a.roomLength())return!1;for(var m in Direction.CROSS2)if(h=g.clone().add(Direction.CROSS2[m]),0<h.x&&h.x<b&&0<h.y&&h.y<c&&(k=a.getDungeonManager().getMap(h.x,h.y),k.type===Terrain.ROOM)){var n=a.getRoom(k.id);if(!d.isRoomConnected(n)&&(k=g.clone().add(.5*Direction.CROSS2[m].x,.5*Direction.CROSS2[m].y),e.isEntrancePossible(a,k))){m=new Entrance(a.getDungeonManager().getEntranceId(),
k.x,k.y);var r=n.roomGroup==l?n:n.roomGroup,u=d.corridorGroup==l?d:d.corridorGroup;m.setCorridor(d);m.setRoom(n);n.roomGroup==l?(n.setEntrance(m),n.setCorridor(d)):(n.roomGroup.setEntrance(m),n.roomGroup.setCorridor(d),n.roomGroup.rooms.forEach(function(a,b,c){u.setRoom(a);return!0}));e.whereIsEntrance(n,h);d.corridorGroup==l?(d.setEntrance(m),d.setRoom(n),g.deadEnd&&(g.deadEnd=!1,d.deleteDeadEnd(g))):(d.corridorGroup.setEntrance(m),d.corridorGroup.setRoom(n),d.corridorGroup.corridors.forEach(function(a,
b,c){r.setCorridor(a);return!0}),g.deadEnd&&(g.deadEnd=!1,d.corridorGroup.deleteDeadEnd(g)));n.roomGroup!=l&&n.roomGroup.rooms.forEach(function(a,b,c){u.setRoom(a);return!0});a.getMap(k.x,k.y)===Terrain.WALL&&(g=a.getDungeonManager().getMap(k.x,k.y),g.type===Terrain.WALL&&a.deleteWall(g.id));a.updateMap(Terrain.ENTRANCE,m.id,k.x,k.y);a.setEntrance(m);break}}return!0});return!0})},isEntrancePossible:function(a,b){var c=a.width-1,e=a.height-1,d;for(d in Direction.CROSS){var k=b.clone().add(Direction.CROSS[d]);
if(0<k.x&&k.x<c&&0<k.y&&k.y<e&&a.getMap(k.x,k.y)===Terrain.ENTRANCE)return!1}return!0},whereIsEntrance:function(a,b){for(var c=!1,e=[a.north,a.south,a.east,a.west],d=0;4>d;d++){for(var k in e[d])if(e[d][k].equals(b)){0===d?a.roomGroup!=l?a.roomGroup.setEntranceOnNorth():a.isEntranceOnNorth=!0:1===d?a.roomGroup!=l?a.roomGroup.setEntranceOnSouth():a.isEntranceOnSouth=!0:2===d?a.roomGroup!=l?a.roomGroup.setEntranceOnEast():a.isEntranceOnEast=!0:3===d&&(a.roomGroup!=l?a.roomGroup.setEntranceOnWest():
a.isEntranceOnWest=!0);c=!0;break}if(c)break}},checkRoomsConnectivity:function(a){var b=a.getRoomKeys(),c=b.length,b=a.getRoom(b[0]),e=[b],d=e[0].getCorridorKeys(),k=[];0!=b.roomLength()&&b.connectedRooms.forEach(function(a,b,c){e.push(a)});0!=b.combinedRoomLength()&&b.combinedRooms.forEach(function(a,b,c){e.push(a)});for(var h in d)k.push(a.getCorridor(d[h]));this.checkRecursively(a,e,k,[]);return c===e.length?!0:!1},checkRecursively:function(a,b,c,e){for(var d=c.length,k=0;k<d;k++){var h=c.shift();
e.push(h);h.connectedRooms.forEach(function(a,d,f){ArrayUtils.containsId(b,a)||(b.push(a),a.connectedCorridors.forEach(function(a,b,d){ArrayUtils.containsId(e,a)||ArrayUtils.uniquePushId(c,a);return!0}),a.connectedRooms.forEach(function(a,d,f){ArrayUtils.uniquePushId(b,a);a.connectedCorridors.forEach(function(a,b,d){ArrayUtils.containsId(e,a)||ArrayUtils.uniquePushId(c,a);return!0});return!0}),a.combinedRooms.forEach(function(a,d,f){ArrayUtils.uniquePushId(b,a);a.connectedCorridors.forEach(function(a,
b,d){ArrayUtils.containsId(e,a)||ArrayUtils.uniquePushId(c,a);return!0});return!0}));return!0})}0!=c.length&&this.checkRecursively(a,b,c,e)},connectRooms:function(a){var b=this;a.rooms.forEach(function(c,d,f){b.isThereARoom(a,c,c.north,new Cell(0,-2));b.isThereARoom(a,c,c.south,new Cell(0,2));b.isThereARoom(a,c,c.west,new Cell(-2,0));b.isThereARoom(a,c,c.east,new Cell(2,0));return!0})},isThereARoom:function(a,b,c,d){for(var f in c){var k=c[f].clone().add(d),h=c[f].clone().add(d.clone().div(2));if(0<
k.x&&k.x<a.width&&0<k.y&&k.y<a.height&&a.getMap(k.x,k.y)===Terrain.ROOM&&a.getMap(h.x,h.y)===Terrain.WALL){var g=a.getRoom(a.getDungeonManager().getMap(k.x,k.y).id);if((b.roomGroup==l||g.roomGroup==l||b.roomGroup.id!==g.roomGroup)&&g!=l){var k=new Entrance(a.getDungeonManager().getEntranceId(),h.x,h.y),p=a.getDungeonManager().getMap(h.x,h.y);a.deleteWall(p.id);a.updateMap(Terrain.ENTRANCE,k.id,h.x,h.y);a.setEntrance(k);b.setRoom(g);g.setRoom(b);b.roomGroup==l&&g.roomGroup==l?(b.connectedCorridors.combineBoth(g.connectedCorridors),
b.connectedEntrances.combineBoth(g.connectedEntrances)):b.roomGroup!=l?(g.connectedCorridors.forEach(function(a,c,d){b.roomGroup.setCorridor(a)}),g.connectedEntrances.forEach(function(a,c,d){b.roomGroup.setEntrance(a)}),b.roomGroup.connectedCorridors.forEach(function(a,b,c){g.setCorridor(a)}),b.roomGroup.connectedEntrances.forEach(function(a,b,c){g.setEntrance(a)})):g.roomGroup!=l?(b.connectedCorridors.forEach(function(a,b,c){g.roomGroup.setCorridor(a)}),b.connectedEntrances.forEach(function(a,b,
c){g.roomGroup.setEntrance(a)}),g.roomGroup.connectedCorridors.forEach(function(a,c,d){b.setCorridor(a)}),g.roomGroup.connectedEntrances.forEach(function(a,c,d){b.setEntrance(a)})):(h=g.roomGroup,b.roomGroup.combine(h),a.deleteRoomGroup(h.id),h.dispose());k.setRoom(b);k.setRoom(g)}}}},createExtraEntrance:function(a){var b=this;a.rooms.forEach(function(c,d,f){d=c.entranceLength();f=Math.round(Math.random()*a.parameters.maxExtraEntrances)+1;for(var k=l;d<f;){var h=f,k=c.roomGroup!=l?b.prioritizeEdges(c.roomGroup):
b.prioritizeEdges(c),g;for(g in k){var p=k[g],q=!1,m=l,n=l,r=p.dir;"north"===r?n=Direction.NORTH:"south"===r?n=Direction.SOUTH:"east"===r?n=Direction.EAST:"west"===r&&(n=Direction.WEST);for(var u in p.data){var t=p.data[u],m=t.clone().add(n);if(b.isExtraEntrancePossible(a,t,n)){p=new Entrance(a.getDungeonManager().getEntranceId(),m.x,m.y);t=t.clone().add(2*n.x,2*n.y);n=a.getDungeonManager().getMap(t.x,t.y);n=a.getCorridor(n.id);t=a.getCell(t.x,t.y);m=a.getDungeonManager().getMap(m.x,m.y);c.roomGroup!=
l?(q=c.roomGroup,q.setEntrance(p),q.setCorridor(n),"north"===r?c.roomGroup.setEntranceOnNorth():"south"===r?c.roomGroup.setEntranceOnSouth():"east"===r?c.roomGroup.setEntranceOnEast():"west"===r&&c.roomGroup.setEntranceOnWest()):(c.setEntrance(p),c.setCorridor(n),"north"===r?c.isEntranceOnNorth=!0:"south"===r?c.isEntranceOnSouth=!0:"east"===r?c.isEntranceOnEast=!0:"west"===r&&(c.isEntranceOnWest=!0));n.corridorGroup!=l?(r=n.corridorGroup,r.setEntrance(p),r.setRoom(c),t.deadEnd&&(t.deadEnd=!1,r.deleteDeadEnd(t))):
(n.setEntrance(p),n.setRoom(c),t.deadEnd&&(t.deadEnd=!1,n.deleteDeadEnd(t)));p.setRoom(c);p.setCorridor(n);a.deleteWall(m.id);a.updateMap(Terrain.ENTRANCE,p.id,p.x,p.y);a.setEntrance(p);f--;q=!0;break}}if(q)break}if(h===f)break}return!0})},isExtraEntrancePossible:function(a,b,c){var d=b.clone().add(2*c.x,2*c.y);return a.getMap(d.x,d.y)===Terrain.CORRIDOR?(b=b.clone().add(c),this.isEntrancePossible(a,b)):!1},prioritizeEdges:function(a){var b=[],c=[a.isEntranceOnNorth,a.isEntranceOnSouth,a.isEntranceOnEast,
a.isEntranceOnWest],d=!1,f=Array(a.north.length),k=Array(a.south.length),h=Array(a.east.length),g=Array(a.west.length);ArrayUtils.copy(a.north,f);ArrayUtils.copy(a.south,k);ArrayUtils.copy(a.east,h);ArrayUtils.copy(a.west,g);ArrayUtils.shuffle(f);ArrayUtils.shuffle(k);ArrayUtils.shuffle(h);ArrayUtils.shuffle(g);for(a=0;2>a;a++){for(var p=0===a?!1:!0,l=0;4>l;l++)c[l]===p&&(0===l?b.push({dir:"north",data:f}):1===l?b.push({dir:"south",data:k}):2===l?b.push({dir:"east",data:h}):3===l&&b.push({dir:"west",
data:g}));0===a&&(0!=b.length?ArrayUtils.shuffle(b):0===b.length&&(d=!0))}d&&ArrayUtils.shuffle(b);return b},createStairs:function(a){var b=[];a.parameters.createStairsAtDeadEnd?a.corridors.forEach(function(a,c,d){a.deadEnds.forEach(function(c,d,e){b.push({id:a.id,data:c});return!0});return!0}):a.rooms.forEach(function(a,c,d){a.cells.forEach(function(c,d,e){b.push({id:a.id,data:c});return!0});return!0});for(var c=MathUtils.randomInt(0,b.length-1),d=MathUtils.randomInt(0,b.length-1),f=b[c].id,k=b[d].id;f===
k;)d=MathUtils.randomInt(0,b.length-1),k=b[d].id;var c=b[c].data,h=new Stair(Terrain.DESCENDING_STAIR,a.getDungeonManager().getStairId(),c.x,c.y),d=b[d].data,g=new Stair(Terrain.ASCENDING_STAIR,a.getDungeonManager().getStairId(),d.x,d.y);a.parameters.createStairsAtDeadEnd?(f=a.getCorridor(f),k=a.getCorridor(k),h.setCorridor(f),g.setCorridor(k),f.corridorGroup!=l?f.corridorGroup.deleteDeadEnd(c):f.deleteDeadEnd(c),k.corridorGroup!=l?k.corridorGroup.deleteDeadEnd(d):k.deleteDeadEnd(d)):(h.setRoom(a.getRoom(f)),
g.setRoom(a.getRoom(k)));a.updateMap(Terrain.DESCENDING_STAIR,h.id,h.x,h.y);a.updateMap(Terrain.ASCENDING_STAIR,g.id,g.x,g.y)},removeDeadEnd:function(a){for(var b=a.getCorridorKeys(),c=b.length,d=0;d<c;d++)for(var f=a.getCorridor(b[d]),k=f.deadEndLength(),h=f.getDeadEndKeys(),g=0;g<k;g++){var l=f.deadEnds.get(h[g]);Math.random()<a.parameters.removeDeadEndRatio&&this.deleteDeadEnd(a,l)}},deleteDeadEnd:function(a,b){if(!(1<b.cellLength())){var c=l,d=b.getCellKeys(),d=b.connectedCells.get(d[0]);a.corridors.forEach(function(a,
d,e){return a.contains(b)?(c=a,!1):!0});c.corridorGroup!=l?c.corridorGroup.deleteDeadEnd(b):c.deleteDeadEnd(b);c.deleteCell(b);d.deleteCell(b);a.updateMap(Terrain.NOTHING,l,b.x,b.y);!this.isEntranceAround(a,d)&&2>d.cellLength()&&this.deleteDeadEnd(a,d)}},isEntranceAround:function(a,b){for(var c in Direction.CROSS){var d=b.clone().add(Direction.CROSS[c]);if(a.getMap(d.x,d.y)===Terrain.ENTRANCE)return!0}return!1},createCorridorWallMap:function(a){a.corridors.forEach(function(b,c,d){b.cells.forEach(function(b,
c,d){for(var e in Direction.BOX)c=b.clone().add(Direction.BOX[e]),a.getMap(c.x,c.y)===Terrain.NOTHING&&(d=new Wall(a.getDungeonManager().getWallId(),c.x,c.y),a.updateMap(Terrain.WALL,d.id,c.x,c.y),a.setWall(d));return!0});return!0})}};var v=function(){this.init.apply(this,arguments)};v.prototype={init:function(a,b,c){if(0===a%2||0===b%2)throw Error("The edges of dungeon must be odd size.");this.version="1.0.0";this.width=a;this.height=b;this.map=[];this.cells=[];this.walls=this.stairs=this.entrances=
this.corridorGroups=this.corridors=this.roomGroups=this.rooms=l;this.dungeonManager=new DungeonManager(a,b);this.parameters={generationMaxTry:5,roomCreationMaxTry:5,minRooms:8,maxRooms:25,combinedRooms:6,minRoomSize:40,maxRoomSize:100,createExtraCombinedRooms:!0,createStairsAtDeadEnd:!1,createExtraEntrance:!0,removeDeadEnd:!0,branchOut:!1,maxExtraEntrances:3,removeDeadEndRatio:.85,corridorCurveRatio:.4,corridorBranchOutRatio:.4};if(c!=l)for(var d in c)this.parameters.hasOwnProperty(d)&&(this.parameters[d]=
c[d]);for(c=0;c<b;c++)for(d=0;d<a;d++)this.cells[d+a*c]=new Cell(d,c)},generate:function(a){if(a!=l&&this.parameters.generationMaxTry<=a)return!1;this.map=Array(this.width*this.height);this.cells=Array(this.width*this.height);this.rooms=new Obj;this.roomGroups=new Obj;this.corridors=new Obj;this.corridorGroups=new Obj;this.entrances=new Obj;this.stairs=new Obj;this.walls=new Obj;this.fill(Terrain.NOTHING);this.dungeonManager.reset();for(var b=0;b<this.height;b++)for(var c=0;c<this.width;c++)this.cells[c+
this.width*b]=new Cell(c,b);DungeonUtils.createRooms(this,0);this.parameters.createExtraCombinedRooms&&DungeonUtils.createExtraRoomsToCombine(this);DungeonUtils.createCorridors(this);this.parameters.branchOut&&DungeonUtils.connectCorridors(this);DungeonUtils.createEntrance(this);DungeonUtils.checkRoomsConnectivity(this)||(DungeonUtils.connectRooms(this),DungeonUtils.checkRoomsConnectivity(this)||this.generate(a!=l?a+1:1));this.parameters.createExtraEntrance&&DungeonUtils.createExtraEntrance(this);
DungeonUtils.createStairs(this);this.parameters.removeDeadEnd&&DungeonUtils.removeDeadEnd(this);DungeonUtils.createCorridorWallMap(this);return!0},getMap:function(a,b){checkRange(this.width,this.height,a,b);return this.map[a+this.width*b]},setMap:function(a,b,c){checkRange(this.width,this.height,b,c);this.map[b+this.width*c]=a},getMapData:function(a,b){checkRange(this.width,this.height,a,b);return this.dungeonManager.getMap(a,b)},setMapData:function(a,b,c,d){checkRange(this.width,this.height,c,d);
this.dungeonManager.setMap(a,b,c,d)},updateMap:function(a,b,c,d){checkRange(this.width,this.height,c,d);this.setMap(a,c,d);this.setMapData(a,b,c,d)},getDungeonManager:function(){return this.dungeonManager},getCell:function(a,b){return this.cells[a+this.width*b]},getRoomKeys:function(){var a=[];this.rooms.forEach(function(b,c,d){a.push(c);return!0});return a},getRoom:function(a){return this.rooms.get(a.toString())},setRoom:function(a){this.rooms.set(a.id.toString(),a)},deleteRoom:function(a){this.rooms.remove(a.toString())},
roomLength:function(){return this.rooms.length},getRoomGroupKeys:function(){var a=[];this.roomGroups.forEach(function(b,c,d){a.push(c);return!0});return a},getRoomGroup:function(a){return this.roomGroups.get(a.toString())},setRoomGroup:function(a){this.roomGroups.set(a.id.toString(),a)},deleteRoomGroup:function(a){this.roomGroups.remove(a.toString())},roomGroupLength:function(){return this.roomGroups.length},getCorridorKeys:function(){var a=[];this.corridors.forEach(function(b,c,d){a.push(c);return!0});
return a},getCorridor:function(a){return this.corridors.get(a.toString())},setCorridor:function(a){this.corridors.set(a.id.toString(),a)},deleteCorridor:function(a){this.corridors.remove(a.toString())},corridorLength:function(){return this.corridors.length},getCorridorGroupKeys:function(){var a=[];this.corridorGroups.forEach(function(b,c,d){a.push(c);return!0});return a},getCorridorGroup:function(a){return this.corridorGroups.get(a.toString())},setCorridorGroup:function(a){this.corridorGroups.set(a.id.toString(),
a)},deleteCorridorGroup:function(a){this.corridorGroups.remove(a.toString())},corridorGroupLength:function(){return this.corridorGroups.length},getEntranceKeys:function(){var a=[];this.entrances.forEach(function(b,c,d){a.push(c);return!0});return a},getEntrance:function(a){return this.entrances.get(a.toString())},setEntrance:function(a){this.entrances.set(a.id.toString(),a)},deleteEntrance:function(a){this.entrances.remove(a.toString())},entranceLength:function(){return this.entrances.length},getWallKeys:function(){var a=
[];this.walls.forEach(function(b,c,d){a.push(c);return!0});return a},getWall:function(a){return this.walls.get(a.toString())},setWall:function(a){this.walls.set(a.id.toString(),a)},deleteWall:function(a){this.walls.remove(a.toString())},wallLength:function(){return this.walls.length},fill:function(a){var b,c;for(c=0;c<this.height;c++)for(b=0;b<this.width;b++)this.setMap(a,b,c)}};d.Mazen=v})(window);
